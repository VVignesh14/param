...
def validate_segwit_address(addr : str):
    return segwit_pattern.match(addr) is not None
    # may need later: 
    match = segwit_pattern.match(addr)
    if match is not None:
        return match.group(0) == addr
    return False

def decode_base58(addr : str):
...
def validate_old_btc_address(addr : str):
